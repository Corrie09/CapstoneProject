...
# Extract parameters
OBSERVED_LTR_SHARE = df['is_ltr_oriented'].mean()
MEAN_RATING = df['rating_index'].mean()
STD_RATING = df['rating_index'].std()
MEAN_EFFORT = df['effort_index'].mean()
MEAN_SIGNAL_CLARITY = df['target_avg_effort'].mean()
STD_SIGNAL_CLARITY = df['target_avg_effort'].std()
...
def success_probability(own_rating: float, own_effort: float, 
    ...
    base_prob = 0.5 - 0.3 * rating_gap  # Harder to match up
    
    # Effort increases probability
    effort_boost = 0.2 * own_effort
    
    # Market quality helps
    market_boost = 0.1 * (market_quality - 0.5)
    
    prob = base_prob + effort_boost + market_boost
...
def expected_utility_from_effort(effort: float, own_rating: float, own_goal: str,
    ...
    if effort == 0.0:
        return -1000  # Exit has large negative utility (outside option)
